...
id_projet = sys.argv[1] # Exemple : id_projet = 128 (id_projet est généré lors de la création d'un projet d'implantation via l'interface.)
caractere_employeur = sys.argv[2] # Exemple : 1 (1 pour oui, 0 pour non.)
...
def calcul_nb_concurrents(label_metier, milieu, rayon_chalandise):
    query = """
        ...
        left join etablissements_cibles_{} d on (
            st_dwithin(c.point_2154, d.point_2154, {}) and
            d.date_fin is null
        )
...
def get_data_inference_survie_etablissements_3_ans(id_projet, query_carreaux, label_metier):
    ...
    query_variables_1 = ", ".join("round({}.valeur::numeric, 3) as {}".format(x, y) for x, y in zip(liste_alias_tables_1, liste_variables_explicatives_cles_1))
    query_variables_2 = ", ".join("round({}.valeur::numeric, 3) as {}".format(x, y) for x, y in zip(liste_alias_tables_2, liste_variables_explicatives_cles_2))
...
def get_data_zone_envisagee_inference_survie_etablissements_5_ans(id_projet, label_metier):
    ...
    query = """
        ...
        select {}, {}, round(avg(e.valeur)::numeric, 3) as prediction_survie_3_ans
        from carreaux_pertinents a
        inner join carreaux_communes b on a.idinspire = b.idinspire
        inner join {}
        inner join {}
...
def get_id_modele(label_metier, score):

    if score == 'opportunite':
            
        query = """
            SELECT MAX(id) FROM entrainements_modeles_nombre_etablissements WHERE fk_metier = '{}'
        """.format(label_metier)
        cur.execute(query)
        id_modele = cur.fetchall()[0][0]
        
    elif score == 'survie_3':
        query = """
            SELECT MAX(id) FROM entrainements_modeles WHERE fk_donnees_entrainement = '3' AND fk_metier = '{}'
        """.format(label_metier)
        cur.execute(query)
        id_modele = cur.fetchall()[0][0]
        ...
        query = """
            SELECT MAX(id) FROM entrainements_modeles WHERE fk_donnees_entrainement = '5' AND fk_metier = '{}'
        """.format(label_metier)
        cur.execute(query)
        id_modele = cur.fetchall()[0][0]
...
def get_modele(label_metier, score):
    ...
    id_modele = get_id_modele(label_metier, score)
...
def calcul_score_opportunite_zone_envisagee(id_projet, label_metier):
    query = """
    with carreaux_pertinents as (
        select a.idinspire
        from insee_carreaux a
    ...
    from carreaux_pertinents a
    inner join res_carreaux_scores b on a.idinspire = b.idinspire and b.score = 'opportunite' AND b.fk_metier = '{}'
    """.format(id_projet, id_projet, label_metier)
    cur.execute(query)
    con.commit()

def calcul_score_opportunite_adresse_envisagee(id_projet, label_metier):
    query = """
    with carreaux_pertinents as (
        select b.idinspire
        from (
...
def main(id_projet):
    ...
    print("Inférence pour le projet {}\n".format(id_projet))
    ...
    # TMP.
    cur.execute(query_carreaux_alentours)
    tmp = cur.fetchall()
    print("Nombre de carreaux à analyser :", len(tmp), "\n")
    ...
    data_inference['taux_motorisation'] = data_inference['taux_motorisation'].fillna(data_inference['taux_motorisation'].mean())
    ...
    variables_explicatives_projet['valeur'] = pd.to_numeric(variables_explicatives_projet['valeur'])
    variables_explicatives_projet['centile']= variables_explicatives_projet.groupby('varexp').valeur.rank(pct=True)
    print(variables_explicatives_projet)
    variables_explicatives_projet = variables_explicatives_projet.replace({'varexp': variables_explicatives})
    ...
    if data_projet['localisation_type'] == 'dessin sur une zone':
        calcul_score_opportunite_zone_envisagee(id_projet, data_projet['label_metier'])
    if data_projet['localisation_type'] == 'adresse':
        calcul_score_opportunite_adresse_envisagee(id_projet, data_projet['label_metier'])
