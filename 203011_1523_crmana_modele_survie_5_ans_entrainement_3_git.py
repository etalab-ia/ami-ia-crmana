...
def get_data_metier_milieu(metier, milieu, relevant_fields):
    ...
    query = """
        select a.siret, b.duree_vie, b.survie_5_ans, c.type_com as type_milieu_2, c.statut_2017 as type_milieu_4
    ...
    df = pd.DataFrame(data, columns=[
        ...
        'indice_concurrence_restreinte', 
        'indice_concurrence_large', 
...
def add_predictions_survie_3_ans(df):
    ...
    nom_fichier_modele = '{}/{}_trained_model'.format(label_metier, id_modele_survie_3_ans)
    fichier_modele = open(nom_fichier_modele, 'rb')
    modele = pickle.load(fichier_modele)
    fichier_modele.close()
    nom_fichier_X_3 = '{}/{}_X'.format(label_metier, id_modele_survie_3_ans)
...
def filter_most_signifiant_features(model, X, y):
    ...
    return X_transformed, liste_chiffres
...
def find_best_models(data, scorer, cv):
    ...
    training_data_survie_5_ans_file_name = '{}/tmp_training_data_survie_5_ans'.format(label_metier) # ***
    training_data_survie_5_ans_file = open(training_data_survie_5_ans_file_name, 'wb') # ***
    pickle.dump(training_data_survie_5_ans, training_data_survie_5_ans_file) # ***
    ...
    simplified_X_5, liste_chiffres = filter_most_signifiant_features(best_rf_model_5, X_5, y_5)
    ...
    # }
    return {
        '5': {'dataset_name': '5', 'X': simplified_X_5, 'y': y_5, 'optimized_models': [{'model_name': 'rf', 'optimized_model': best_rf_model_5}], 'liste_chiffres': liste_chiffres}
    }
...
def save_in_database(to_be_saved):
    ...
    query = """
        insert into entrainements_modeles(fk_metier, date, fk_donnees_entrainement, fk_modele, parametres, accuracy, rapport_classification, confusion_matrix, chiffres_selection_variables)
        values ('{}', now(), '{}', '{}', '{}', {}, '{}', '{}', '{}');
    """.format(
        ...
        ' ; '.join(to_be_saved['liste_chiffres'])
    )
    cur.execute(query)
    con.commit()
